...
We extend our protein benchmarks to protein domains called bromodomains, which have recently been the topic of numerous studies in the search for high affinity ligands, using both computational and structure-based techniques. In living cells, bromodomains are responsible for recognizing and binding to acetylated lysine residues present in the histone tails, a known epigenetic marker. They also bind a broad spectrum of small to medium-size molecules with micro and nanomolar affinities, being a promising target for the treatment of conditions such as cancer and inflammation. Bromodomains can be expressed, purified and studied as independent proteins, which greatly facilitates the computational and experimental determination of ligand affinities. There are a total of 61 bromodomains identified in the human proteome, belonging to 46 different proteins. They all have about about 110 aminoacids in length and, despite having large sequence variations, share a common fold, consisting of four anti-parallel $\alpha$-helices (named $\alpha$Z, $\alpha$A, $\alpha$B, and $\alpha$C) folded into a left-handed bundle (Fig. 3A). The first two and last two helices are connected by the ZA and BC-loops, respectively. These loops are more flexible and make part of the acetyl-lysine binding site, also displaying different lengths and sequences amongst the various bromodomains. 

In the present benchmark we are going to focus on the first bromodomain of the bromodomain-containing protein 4 (BRD4), part of the  BET (bromodomain and extra-terminal domain) family, the latter also encompassing the BRD2, BRD3, and BRDT proteins. The BRD4(1) bromodomain is the most widely studied bromodomain in the BET family, with high-resolution cocrystal structures available for more than a hundred ligands, most of them accompanied by experimental affinities. Small molecule inhibitors of BRD4(1) show great variety, including distinct groups such as triazole, pyrimidine, acetyl-pyrrole, and isoxazole. Due to the hydrophobic nature of the BRD4(1) binding site, all ligands are neutral, and many of them are also relatively rigid with the presence of single, double and triple rings. This reduces the conformational space of the substrate in the bound and unbound states, providing quicker convergence, and partially avoiding symmetry issues that might appear for ligands with many torsional degrees of freedom. 

In contrast to the T4 lysozyme, all molecules bind to the surface of BRD4(1), making it particularly suitable for free energy calculations that rely on a physical pathway. Additionally, its folded state has shown remarkable stability over $\mu$s scale simulations, which allows for exhaustive (or brute-force) sampling with less risk of unfolding or irreversible deformation happening on the protein structure. These characteristics make the BRD4(1) system a strong candidate for the first protein hard benchmark, but a few important issues have to be addressed in order to make this assertion. They involve the flexibility of the ZA-loop in the presence and absence of the substrate, and possible slow side-chain dihedral rotations near the binding site. Below we describe these challenges in more detail, and how they can be overcome. 

\subsubsection{Flexibility and transition of the ZA-loop}

Simulations have shown that the backbone of the BRD4(1) bromodomain in complex with different ligands shows great stability, with reported backbone RMSD values being less than 1.00 {\AA} over simulations lasting up to 120ns. The same behavior was reproduced in simulations over 300 ns long (Heinzelmann et al., unpublished results), with most of the fluctuations happening in the N and C-termini, which are far from the ligand binding site and can be disregarded. In contrast, during 0.5 $\mu$s of simulations of the apo BRD4(1) performed by Steiner et al, large backbone RMSD values were observed in the ZA-loop region. Here, a sharp transition is seen in this loop in the beginning of the simulation, suggesting an initial metastable state and subsequent relaxation of the protein. This trend has been reproduced by later studies, with all of them displaying a quick transition followed by the stabilization of the ZA-loop.

Heinzelmann et al. have investigated this phenomenon in more detail, with simulations and free energy calculations showing that initial apo state of the protein is indeed metastable, and after around 50 ns of simulations a transition is seen from the so-called closed to the open state (Fig. 3B). According to the calculations performed using the Amberff14SB protein force field and the TIP3P water model, the latter is  $\sim$2.5 kcal/mol more favorable than the former. Interestingly, the open state described in this reference does not agree with both apo crystal structures of BRD4(1), which have an identical backbone conformation as the complexes (closed conformation). This discrepancy might arise from simulation artifacts due to the chosen force-field, or, on the other hand, be caused by spurious contacts involving loop residues and neighbouring molecules in the crystal arrangement. This latter possibility has been proposed before, when noting the discrepancy between the crystallographic B-factors and the flexibility of the ZA-loop observed in simulations of the apo CREBBP bromodomain. 

Regardless of the true origin of the observed conformational change, its contribution has to be included in the calculation of the $absolute$ binding free energies of different ligands to BRD4(1), if we wish to obtain results that are consistent with the Amberff14SB force field. Since this transition happens in the absence of the ligands, it works only as a free energy offset, not influencing the $relative$ binding free energies as long as the transition from the closed to the open state is avoided during the calculations. With this in mind, the results relative to the metastable closed state can be obtained by applying a restraint-wall scheme on the Asp88 $\psi$ dihedral, as described in Ref., so they are properly compared for each benchmark molecule. Alternatively, to obtain the ligand binding free energies relative to the apo open state, the free energy of the closed to open transition can also be calculated. As shown in Ref, this transition works through a double hinge mechanism involving the Asp88 and Asp96 residues, and it is tractable using a single reaction coordinate (the Asp88 $\psi$ dihedral), making it suitable for the application of various enhanced sampling methods. 
