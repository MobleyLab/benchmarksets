...
Simulations starting from the cocrystal structures of BRD4(1) bound to different ligands show great stability, with reported backbone RMSD values being less than 1.00 {\AA} over simulations lasting up to 120ns. The same behavior was reproduced in simulations over 300 ns long (Heinzelmann et al., unpublished results), with most of the fluctuations happening in the N and C-termini, which are far from the ligand binding site and can be disregarded. Even though the apo crystal structure of BRD4(1) has the same conformation as the complexes, long simulations of this system using different force fields have shown considerable backbone RMSD values in the ZA-loop region. However, instead of large random fluctuations, all cases show a sharp irreversible transition on this loop generally after 20 ns of simulations, suggesting an initial metastable state followed by stabilization of the protein backbone. 

Heinzelmann et al. have investigated this phenomenon in more detail, with simulations and free energy calculations showing that apo crystal structures of BRD4(1) are indeed metastable, and after around 50 ns of simulations a transition is seen from the so-called closed to the open state (Fig. 3B). According to the calculations performed using the Amberff14SB protein force field and the TIP3P water model, the latter is  $\sim$2.5 kcal/mol more favorable than the former. This discrepancy between the crystal data and the simulations might arise from artifacts due to the simulation force-field, or, on the other hand, be caused by spurious contacts involving loop residues and neighbouring molecules in the crystal arrangement. This latter possibility has been proposed before, when noting the disagreement between the crystallographic B-factors and the flexibility of the ZA-loop observed in simulations of the apo CREBBP bromodomain. 

Regardless of the true origin of the observed conformational change, its contribution has to be included in the calculation of the $absolute$ binding free energies of different ligands to BRD4(1), if we wish to obtain results that are consistent with the Amberff14SB force field. Since this transition happens in the apo state, it works only as a free energy offset and do not impact the ligand $relative$ binding free energies, as long as the transition from the closed to the open state is avoided during the calculations. With this in mind, the results relative to the metastable closed state can be obtained by applying a simple restraint-wall scheme on the Asp88 $\psi$ dihedral, as described in Ref., so they are properly compared for each benchmark molecule. Alternatively, to obtain the ligand binding free energies relative to the apo open state, the free energy of the closed to open transition can also be calculated. As shown in Ref, this transition works through a double hinge mechanism involving the Asp88 and Asp96 residues, and it is tractable using a single reaction coordinate (the Asp88 $\psi$ dihedral), making it suitable for the application, testing and comparison of various enhanced sampling methods. 





\subsubsection{Binding free energy calculations on BRD4(BD1)}

Many studies have focused on the calculation of binding free energies of small molecules to BRD4(BD1), using a variety of methods such as alchemical transformations, attach-pull-release (APR), adaptive biasing force (ABF), steered molecular dynamics (SMD), molecular mechanics combined with either Poisson-Boltzmann (MM-PBSA) or generalized Born (MM-GBSA) surface area, and simple scoring functions. The first two methods have provided the most rigorous calculations performed on the BRD4(BD1) system so far. They combine all-atom simulations with the application of restraints, in order to capture all the relevant degrees of freedom of the protein-ligand system in both the bound and the unbound states. 

Aldeghi et al. have used alchemical calculations to determine the binding free energy of 11 ligands to the first BRD4 bromodomain, obtaining encouraging agreement with experimental values. When starting from the crystal structures of the complexes, the comparison to the experimental affinities yielded a Pearson's correlation coefficient of $0.84\pm0.05$ and a root mean square error (RMSE) of $0.8\pm0.2$ kcal/mol, with the 11 ligands spanning a $\sim$4 kcal/mol range. This rigorous alchemical approach was also able to determine the correct binding mode of the RVX-OH and RVX-208 ligands, when starting from multiple possibilities generated by docking. A subsequent study has extended it to other bromodomains, in order to test these calculations for both pose prediction and ligand selectivity. The binding free energies for RVX-OH and RVX-208 still showed good correlation with experiment, but the ones from bromosporine were less successful. 

More recently, Heinzelmann et al. used the attach-pull-release protocol to calculate the binding free energies of seven ligands to BRD4(BD1), using different water models and ligand parameters. In contrast to alchemical transformations, APR relies on a physical path to pull the ligand from the binding site, by using umbrella sampling. Due to the much longer simulations performed on this study when compared to (ref), it was found that the bound conformation of BRD4(BD1) is only metastable when the ligand is removed from the binding site. Typically after $\sim$50ns of simulations, the apo protein transitions from the closed to the open state, which will be explained in more detail in the next section. The free energy contribution of this transition has to be included in the calculations, so that the ligand binding free energies are consistent with the system initial (bound) and final (unbound) states in the absence of restraints.

Interestingly, the open state does not agree with both apo BRD4(BD1) crystal structures, which are in the closed state, and the conformational transition of the apo protein was not observed during the calculations from (Ref) (personal communication by Biggin), due to the shorter simulation times (10ns per window). Therefore, in Ref (mine) two sets of binding free energies were calculated: one relative to the metastable apo closed state, and the other to the final apo open state. In both cases, the SPC/E water model combined with GAFF ligand parameters produced the best agreement with experimental values for the 7 ligands, with a Pearson's correlation coefficient of $0.83\pm0.16$ for both final apo states, and RMSE values of $1.42\pm0.29$ kcal/mol and $1.29\pm0.36$ kcal/mol for the open and closed states, respectively. 

In Ref(mine) there is also comparison between calculations performed using APR and the alchemical approach for three of the ligands. When the same water model and ligand parameters are used, as well as the closed final apo state of the protein, excellent agreement is found, with the calculated binding free energies all within $0.3$ kcal/mol between the two studies. Even though the protein force fields are slightly different, Amber99SB-ILDN on Ref(Ald) versus Amberff14SB on Ref(mine), the differences fall inside the associated uncertainties of both methods. This is an indication of the robustness of free energy techniques on this system if they are carried out rigorously, making BRD4(BD1) a good test system to apply and compare various enhanced sampling methods.
