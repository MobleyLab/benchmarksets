...
to 120ns~\cite{Su:2017:J.Biomol.Struct.Dyn.}. The same behavior was reproduced in simulations over 300 ns long (Heinzelmann et al., unpublished results), with most of the fluctuations happening in the N and C-termini, which
are far from the ligand binding site and can be disregarded. Even though the apo crystal structures of BRD4(1) have the same conformation as the complexes~\cite{Lucas:2013:Angew.Chem.Int.Ed., Filippa:2012:Cell}, long simulations 
of this system using different force fields have shown considerable backbone RMSD values in the ZA-loop region~\cite{Su:2017:J.Biomol.Struct.Dyn., Kuang:2015:J.Chem.Inf.Model., Steiner:2013:FEBSLett.}. However, instead of large random
fluctuations, all cases show a sharp irreversible transition on this loop generally after 20 ns of simulations, suggesting an initial metastable state followed by stabilization of the protein backbone. 

Heinzelmann et al.~\cite{Heinzelmann:2017:J.Chem.TheoryComput.} have investigated this phenomenon in more detail, with simulations and free energy calculations showing that apo crystal structures of BRD4(1) are indeed metastable, 
and after around 50 ns of simulations a transition is seen from the so-called closed to the open state (Fig. 3B). According to the free energy calculations performed using the Amberff14SB protein force field and the TIP3P water model, 
the latter conformation is  $\sim$2.5 kcal/mol more favorable than the former. This discrepancy between the crystal data and the simulations might arise from artifacts due to the simulation force-field, or, on the other hand, be caused by spurious 
contacts involving loop residues and neighbouring molecules in the crystal arrangement. This last possibility has been proposed before, when noting the inconsistency between the relatively uniform crystallographic B-factors and the high 
flexibility of the ZA-loop observed in simulations of the apo CREBBP bromodomain~\cite{Spilio:2014:Isr.J.Chem.}. 

The free energy of the apo BRD4(1) closed-to-open conformational change is independent of the substrate~\cite{Heinzelmann:2017:J.Chem.TheoryComput.}, so it will not impact the ligands $relative$ binding free energies, as long
as this transition is avoided during the calculations. This would be the case when performing small alchemical transformations on the substrate inside the binding pocket~\cite{schrodinger_accurate_2015}, since the BRD4(1) bound (closed)
state would be preserved due to the presence of the ligand. For the calculation of the ligands $absolute$ binding free energies, the conformational change of the apo BRD4(1) will influence the results and has to be taken into account.
As we explain below, the absolute affinities for the benchmark ligands can be computed either relative to the metastable apo closed state, consistent with the apo crystal structures of BRD4(1), or the apo open state, consistent with the
Amberff14SB force-field. 

The ligand binding free energies relative to the metastable apo closed state of BRD4(1) are calculated by including a simple restraint-wall scheme on the Asp88 $\psi$ dihedral, as described in 
Ref.~\cite{Heinzelmann:2017:J.Chem.TheoryComput.}, so they are properly compared for each benchmark molecule. Alternatively, to determine the absolute ligand binding free energies relative to the apo open state, 
the free energy of the apo closed-to-open transition can also be determined. As shown by Heinzelmann et al.~\cite{Heinzelmann:2017:J.Chem.TheoryComput.}, this transition works through a double hinge mechanism centered on the 
Asp88 and Asp96 residues, and it is tractable using a single reaction coordinate (the Asp88 $\psi$ dihedral), making it suitable for the application, testing and comparison of various enhanced sampling methods. 
...
which are part of the acetyl-lysine binding site and are located on the ZA and BC-loops, respectively. The side-chain movements of these two residues were first observed on simulations of the apo CREBBP 
...
Tyr1125 (Tyr97 on BRD4(1)). On the other hand, as in Refs.~\cite{Steiner:2013:FEBSLett., Spilio:2014:Isr.J.Chem.}, rotations on the Asn140 $\chi_2$ dihedral frequently take place in simulations of the apo BRD4(1), but they are found to
be reversible and happening in a nanosecond time scale (Fig. 3D). Still, we cannot discard that slower side-chain transitions might still occur on BRD4(1) in the absence of the ligand, which would have to be accounted for if one is looking
for gold standard results for this system.
