...
We believe applications of free energy calculations to small protein domains called bromodomains can provide a suitable first benchmark on systems that have pharmaceutical relevance, thus, here, we include a bromodomain benchmark. 
...
As in the case of T4 lysozyme, most BRD4(1) inhibitors are relatively rigid and neutral, with clear protonation states~\cite{Filippa:2014:Nat.Rev.DrugDiscov.}. 
...
Protein conformational sampling presents a major hurdle for BRD4(1) binding studies. 
The ligand complexity can be reduced by choosing small and rigid ligands for a benchmark set, but even then, protein conformational sampling presents a challenge.  
...
In living cells the BRD4(1) bromodomain binds to acetylated lysines present in the histone tails~\cite{Chung:2016:Prog.Med.Chem.}, but it is also capable of binding a broad spectrum of small molecules affinities as strong as nanomolar~\cite{Filippa:2014:Nat.Rev.DrugDiscov.}.
...
They also have a broad range of binding free energies, with affinities spanning a $\sim$6 kcal/mol range~\cite{Vidler:2013:J.Med.Chem.,Gehling:2013:ACSMed.Chem.Lett.}, and with crystal structures available for many. 
Charged ligands are possible~\cite{aldeghi_accurate_2016}, but most are neutral, with many inhibitors having double and triple rings.  %Double and triple rings -- what does this mean? Bicyclic and tricyclic ring systems? Or two or three rings per inhibitor? I'm thinking you mean bicyclic and tricyclic?
This increases rigidity of the ligands and reduces the conformational freedom of the inhibitors in the bound and unbound states, providing quicker convergence for free energy calculations, and partially avoiding symmetry issues that might appear for ligands with a large number of torsional degrees of freedom~\cite{mobley_use_2006}. 
...
%DLM: I tried to soften the language above slightly to be a little less definitive. I think you're right, but I don't want to be super definitive about it. Does the rewording look OK?

{\bf Table~\ref{brd4_benchmark}} shows our proposed initial benchmark set for BRD4(1), including one verified non-binder, with experimental binding free energies spanning a $\sim$6 kcal/mol range. 
...
For this initial benchmark set all ligands are neutral, but they present different levels of complexity, ranging from fairly rigid molecules to molecules with several rotating groups. 
...
Even though our set initially is considered a soft benchmark, we are optimistic that further application of free energy methods to these cases, especially with a focus on enhanced conformational sampling in the bound and unbound states, can turn this diverse set of ligands in to the first protein hard benchmark.  
...
(D) Time series of the Asn140 $\chi_2$ dihedral during 350 ns of simulations of the \emph{apo} BRD4(1), showing frequent and reversible transitions. %DLM: Can we say exactly which simulations? e.g. are these the ones from your published work? If so you could say ``(simulation details given in [ref])''
}
\end{figure*}
...
The open state appears to be $\sim$2.5 kcal/mol more favorable than the closed state, according to free energy calculations performed using the AMBER ff14SB protein force field and the TIP3P water model. %DLM: Is this in your study? If so please make clear, such as by giving the ref, or saying "in the aforementioned study" or similar.
...
That way the conformational transition is avoided during the calculations, so the results are all relative to the same final state of the protein. 
...
Still, we it is possible that slower side-chain transitions might still occur on BRD4(1) in the absence of the ligand, which would have to be accounted for if one is looking for gold standard results for this system. 
