...
In order to provide a detailed description of systems that have pharmaceutical relevance, we also include in our benchmarks small protein domains called bromodomains. These structures have been the subject of several experimental~\cite{Zhang:2013:J.Med.Chem.,Bamborough:2015:J.Med.Chem., Ran:2015:J.Med.Chem., Gosmini:2014:J.Med.Chem., Chung:2011:J.Med.Chem., Xue:2016:J.Med.Chem., Fill:2012:Bioorg.Med.Chem., Fish:2012:J.Med.Chem.} and computational~\cite{Deepak:2017:Chem.Biol.Drug.Des., Dickson:2016:J.Chem.Phys., Raj:2016:J.Biomol.Struct.Dyn., Ran:2015:Mol.Biosyst., Zhao:2014:Bioorg.Med.Chem.Lett, Xu:2016:J.Med.Chem., Yang:2016:BioorganicChemistry} studies in the search for high affinity ligands, in particular the first bromodomain of the BRD4 protein, or BRD4(1).
As in the T4 lysozyme, most BRD4(1) inhibitors are relatively rigid, neutral and with clear protonation states~\cite{Filippa:2014:Nat.Rev.DrugDiscov.}. 
When it comes to the application and comparison of binding free energy methods, BRD4(1) possesses a few advantages over T4, such as ligands with greater variety and a binding site with clear access to the solvent~\cite{Filippa:2014:Nat.Rev.DrugDiscov., Fill:2012:Bioorg.Med.Chem., Xue:2016:J.Med.Chem.}. 
There is a large number of high-quality cocrystal structures and experimental affinities available for this system, as well as theoretical results that show encouraging agreement with experiments~\cite{aldeghi_accurate_2016,Aldeghi:2017:J.Am.Chem.Soc.,Heinzelmann:2017:J.Chem.TheoryComput.}. 
Even with the apparent convergence of binding free energy calculations on BRD4(1), they still present a few challenges, most them related to the conformational sampling of the protein in the bond and unbound states. 
We believe that a careful analysis of these issues, combined with rigorous free energy calculations, have the potential of making the BRD4(1) bromodomain the first protein hard benchmark.


Even though we can reduce the ligand complexity by choosing small and rigid molecules, the protein conformation still remains the biggest challenge for well-converged results in the BRD4(1) system. 
The bound state of BRD4(1) in complex with different ligands has shown long-term stability in simulations at room temperature, but the \emph{apo} state exhibits increased flexibilty, particularly in the ZA-loop (Fig. ~\ref{fig:brd4}(A)) ~\cite{Su:2017:J.Biomol.Struct.Dyn., Steiner:2013:FEBSLett.}. 
...
Indeed, a conformational change in the ZA-loop in the absence of the ligand was observed by independent studies~\cite{Heinzelmann:2017:J.Chem.TheoryComput., Kuang:2015:J.Chem.Inf.Model., Steiner:2013:FEBSLett.}, and this is likely to have a significant effect in the calculated binding free energies. 
In addition, long simulations of the \emph{apo} CREBBP bromodomain suggest that slow side-chain rearrangements are also possible on BRD4(1)~\cite{Steiner:2013:FEBSLett., Spilio:2014:Isr.J.Chem.}, which can be another complicating factor. 
As we show below, each these challenges can be properly addressed, making them an integral part of the binding free energy calculations.  


\subsubsection{The first bromodomain of BRD4 and its ligands}

In living cells the BRD4(1) bromodomain binds to acetylated lysines present in the histone tails~\cite{Chung:2016:Prog.Med.Chem.}, but it is also capable of binding a broad spectrum of small molecules with nano and micromolar affinities~\cite{Filippa:2014:Nat.Rev.DrugDiscov.}.
Although the binding site of BRD4(1) has a hydrophobic character, most of its strong inhibitors engage in a hydrogen bond with the Asn140 residue, mimicking the binding mode of the acetylated lysine. 
...
They also have a broad range of binding free energies, with available crystal structures and affinities spanning a $\sim$6 kcal/mol range~\cite{Vidler:2013:J.Med.Chem.,Gehling:2013:ACSMed.Chem.Lett.}. 
Charged ligands are possible~\cite{aldeghi_accurate_2016}, but most are neutral, with the common presence of double and triple rings. 
This increases rigidity and reduces the conformational space of the substrate in the bound and unbound states, providing quicker convergence, and partially avoiding symmetry issues that might appear for ligands with a large number of torsional degrees of freedom~\cite{mobley_use_2006}. 

In contrast to T4 lysozyme, all BRD4(1) ligands bind to its surface~\cite{Filippa:2014:Nat.Rev.DrugDiscov., Fill:2012:Bioorg.Med.Chem., Xue:2016:J.Med.Chem.}, making this target more suitable for free energy calculations that rely on a physical pathway~\cite{Heinzelmann:2017:J.Chem.TheoryComput., Kuang:2015:J.Chem.Inf.Model., Muvva:2014:Mol.Biosyst.}. 
This has allowed the application of methods that involve both alchemical transformations~\cite{aldeghi_accurate_2016, Aldeghi:2017:J.Am.Chem.Soc.} and the physical pulling of the ligand from the binding site, such as the APR method~\cite{Heinzelmann:2017:J.Chem.TheoryComput.}. 
The alchemical approach is capable of providing root-mean-squared errors (RMSE) as low as $0.8\pm0.2$ kcal/mol and Pearson's correlation coefficient over $0.80$ for ligands spanning a $\sim$4 kcal/mol range. 
Similar correlation is achieved using APR, but with slightly larger RMSEs.
The two methods are also able to predict the correct binding pose of a ligand starting from multiple possibilities generated by docking, which is another application of free energy calculations~\cite{Kaus:2015:J.Chem.TheoryComput.}. 
An important difference between the APR~\cite{Heinzelmann:2017:J.Chem.TheoryComput.} and the alchemical calculations~\cite{aldeghi_accurate_2016} on BRD4(1) is that the former also included the free energy contribution from a conformational change in the protein in the absence of the ligand, whereas in the latter study this transition was not observed (personal communication by Biggin). 

When the APR and the alchemical approaches are compared relative to the same \emph{apo} state of BRD(1) for three different ligands~\cite{Heinzelmann:2017:J.Chem.TheoryComput.}, excellent agreement is found, with the discrepancies observed all within the associated uncertainties of the two methods. 
Although slightly different protein force fields were used, Amber99SB-ILDN on Ref.~\cite{aldeghi_accurate_2016} versus Amberff14SB on Ref.~\cite{Heinzelmann:2017:J.Chem.TheoryComput.}, this is an indication of the robustness of free energy techniques one can expect from this system. 
The fact that the APR study used much longer simulation times shows that convergence for this system can be obtained relatively quickly, aside from the BRD4(1) transition in the \emph{apo} state which will be discussed in the next section. 

{\bf Table~\ref{brd4_benchmark}} shows the initial benchmark set for the BRD4(1) system, including one verified non-binder, and experimental binding free energies spanning a $\sim$6 kcal/mol range. 
Some ligands are similar and can be treated with relative binding free energy calculations, but others present significant differences, being more suitable for absolute calculations.  
There are crystal structures available for all ligands except the non-binder, for which there is an available structure for a very similar ligand.   
For the initial benchmarks all ligands are neutral, but they present different levels of complexity, since fairly ridig molecules to molecules with several rotating groups. 
Except for the first two, all the other ligands have already been treated with the alchemical and/or APR methods, including the three ligands for which these calculations were compared.   
Even though our set initially is considered a soft benchmark, the application of free energy methods to the protein/ligand conformations in the bound and unbound states can turn this diverse set of ligands in to the first protein hard benchmark.  
...
The two hinges, Asp88 and Asp96, are shown in green. (C) The same structures of item B, but showing the side chains of the  Tyr97, Tyr139 and Asn140 residues, which remain unaffected. 
(D) Time series of the Asn140 $\chi_2$ dihedral during 350 ns of simulations of the \emph{apo} BRD4(1), showing frequent and reversible transitions. 
}
\end{figure*}
...
\midrule
Compound 8a                  & 2295889                                 & \parbox[c]{1em}{\includegraphics[scale=0.2]{figures/new-bromo/2295889.pdf}}     &                             & NB$^{\rm d}$                                                   & ND$^{\rm c}$                      & ~\cite{Vidler:2013:J.Med.Chem.}         \\ %included for symmetry with L99A site
Compound 4                             & 975713                                & \parbox[c]{1em}{\includegraphics[scale=0.2]{figures/new-bromo/975713.pdf}}      &                           & -4.49$^{\rm a,f}$                                                  & 4MEQ~\cite{Vidler:2013:J.Med.Chem.}        & ~\cite{Vidler:2013:J.Med.Chem.}         \\ % relative set we did
XD1                 & 234105                                  & \parbox[c]{1em}{\includegraphics[scale=0.2]{figures/new-bromo/234105.pdf}}      &                        & -5.95$^{\rm b}$                                            & 4LYS~~\cite{Lucas:2013:Angew.Chem.Int.Ed.}    & ~\cite{Lucas:2013:Angew.Chem.Int.Ed.}         \\ % relative set we did
Quinazolin           & 2927305                                & \parbox[c]{1em}{\includegraphics[scale=0.2]{figures/new-bromo/2927305.pdf}}    &                          & -6.36\pm0.17$^{\rm a}$                                            & 4HBV~\cite{Fish:2012:J.Med.Chem.} & \cite{Fish:2012:J.Med.Chem.}  \\% relative set we did
Alprazolam        & 2118                                 & \parbox[c]{1em}{\includegraphics[scale=0.2]{figures/new-bromo/2118.pdf}}     &                      & -7.40\pm0.05$^{\rm b}$                                            & 3U5J~\cite{Fill:2012:Bioorg.Med.Chem.} & \cite{Fill:2012:Bioorg.Med.Chem.}  \\ %induces helix motions
RVX-208 & 24871506                             & \parbox[c]{1em}{\includegraphics[scale=0.2]{figures/new-bromo/24871506.pdf}} &                      & -7.84\pm0.06$^{\rm b}$                                            & 4MR4~\cite{Picaud:2013:PNAS} & \cite{Picaud:2013:PNAS}  \\ % induces helix motion
Bzt-7   & 20350618                                & \parbox[c]{1em}{\includegraphics[scale=0.2]{figures/new-bromo/20350618.pdf}}    &                        & -8.16\pm0.03$^{\rm b}$                                            & 3U5L~\cite{Fill:2012:Bioorg.Med.Chem.} & \cite{Fill:2012:Bioorg.Med.Chem.}  \\ 
RVX-OH                     & N/A                                 & \parbox[c]{1em}{\includegraphics[scale=0.2]{figures/new-bromo/RVX-OH.pdf}}     &                       & -8.99\pm0.07$^{\rm b}$                                                   & 4MR3~\cite{Picaud:2013:PNAS}        & \cite{Picaud:2013:PNAS}         \\ 
JQ1(+)                   & 46907787                                  & \parbox[c]{1em}{\includegraphics[scale=0.2]{figures/new-bromo/46907787.pdf}}      &                          & -9.64\pm0.02$^{\rm b}$                                          & 3MXF~\cite{Filippakopoulos:2010:Nature}                      & \cite{Filippakopoulos:2010:Nature}  \\ % relative set
Coumpound 3                    & N/A                                 & \parbox[c]{1em}{\includegraphics[scale=0.2]{figures/new-bromo/C-3.pdf}}     &                           & -10.41\pm0.23$^{\rm a}$                                          & 4LRG~\cite{Gehling:2013:ACSMed.Chem.Lett.}                     & \cite{Gehling:2013:ACSMed.Chem.Lett.}         \\ 
\bottomrule
\end{tabular}                                                                                                                                                                                                                                                                                                       \\
\end{center}
$^{\rm a}$ Obtained using Alphascreen; $^{\rm b}$ Obtained using Isothermal Titration Calorimetry; $^{\rm c}$ not determined; $^{\rm d}$ nonbinder; $^{\rm e}$ PubChem compound ID.; $^{\rm f}$ Extrapolated from 32\% inhibition at 250$\mu$M 
\end{table*}
\endgroup
...
Further work found that the ZA-loop conformation seen in the \emph{apo} crystal structures (closed state) is metastable and transitions to an \emph{apo} open state (Fig. ~\ref{fig:brd4}(B)) after around 50 ns~\cite{Heinzelmann:2017:J.Chem.TheoryComput.}. 
...
This discrepancy between the crystal data and the simulations might arise from artifacts due to the simulation force field, or, on the other hand, be caused by crystal contacts involving loop residues and neighbouring molecules. 
This last possibility has been proposed before, when noting the inconsistency between the relatively uniform crystallographic B-factors and the high flexibility of the ZA-loop observed in simulations of the \emph{apo} CREBBP bromodomain~\cite{Spilio:2014:Isr.J.Chem.}. 

The free energy of the \emph{apo} BRD4(1) closed-to-open conformational change is independent of the substrate~\cite{Heinzelmann:2017:J.Chem.TheoryComput.}, so it will not impact the ligands' \emph{relative} binding free energies, as long as this transition is avoided during the calculations. 
This would be the case when performing small alchemical transformations on the substrate inside the binding pocket~\cite{schrodinger_accurate_2015}, since the BRD4(1) bound (closed) state will likely be preserved due to the presence of the ligand. Even though this seems to be the case for most molecules, we cannot exclude the possibility of the open conformation of BRD4(1) being more favorable in the bound state for a particular ligand. This would require an additional conformational contribution to be included in the relative binding free energies.
...
The affinities can be computed either relative to the metastable \emph{apo} closed state, consistent with the \emph{apo} crystal structures of BRD4(1), or the \emph{apo} open state, consistent with the Amberff14SB force-field. 
The results for a set of ligands will differ only by an free energy offset depending on this choice. 
The values relative to the metastable \emph{apo} closed state of BRD4(1) are calculated by including a simple restraint-wall scheme on the Asp88 $\psi$ dihedral, as described in  Ref.~\cite{Heinzelmann:2017:J.Chem.TheoryComput.}. 
That way the transiton is avoided during the calculations, so the results are all relative to the same final state of the protein. 
Alternatively, for the absolute ligand binding free energies relative to the \emph{apo} open state, the free energy of the closed-to-open transition can also be determined.
...
In these simulations, the conserved Tyr1167 residue in the BC-loop (equivalent to Tyr139 on BRD4(1)) is shown to adopt an outward conformation towards the solvent, followed by the occupation of the binding site by the side-chain of the conserved Tyr1125 located in the ZA-loop (Tyr97 on BRD4(1)). 
The hydroxyl group from this last residue would mimick the acetyl lysine, making a hydrogen bond with the conserved Asn1168 residue (Asn140 on BRD4(1)). 
...
On the other hand, as in the \emph{apo} CREBBP bromodomain~\cite{Steiner:2013:FEBSLett., Spilio:2014:Isr.J.Chem.}, rotations on the Asn140 $\chi_2$ dihedral frequently take place in simulations of the \emph{apo} BRD4(1), but they are found to be reversible and happening in a nanosecond time scale (Fig. ~\ref{fig:brd4}(D)). 
Still, we cannot neglect the possibility that slower side-chain transitions might still occur on BRD4(1) in the absence of the ligand, which would have to be accounted for if one is looking for gold standard results for this system. 
As in the case of the T4 lysozyme, this can be accomplished by using enhanced sampling techniques such as umbrella sampling~\cite{mobley_confine_2007}. 
